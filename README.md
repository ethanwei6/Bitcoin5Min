...
- Live data probes for both concurrent REST polling and Polymarket's market
  WebSocket stream.
...
Probe live Polymarket and spot-data cadence at one-second resolution:

```bash
PYTHONPATH=src PYTHONPYCACHEPREFIX=work/pycache \
python3 scripts/probe_live_market_data.py \
  --config config/paper_btc_5m.json \
  --duration-seconds 120 \
  --interval-seconds 1 \
  --output-dir reports/live_data_probes
```

This records raw samples plus a summary of CLOB book-hash changes, top-of-book
changes, spot-source spread, error ticks, and component latencies. It answers
whether the live polling path is fresh enough to support five-minute markets; it
does not place trades.

Probe the lower-latency Polymarket market WebSocket:

```bash
node scripts/probe_polymarket_websocket.mjs \
  --config config/paper_btc_5m.json \
  --duration-seconds 120 \
  --output-dir reports/live_data_probes
```

This subscribes to the current BTC 5m market's UP/DOWN token IDs and records
raw `book`, `price_change`, `best_bid_ask`, `last_trade_price`, and resolution
events when Polymarket emits them. The report also grades whether observed
message cadence and timestamp lag are fast enough for the configured bot sample
interval. The REST probe is useful for paper-run comparability; the WebSocket
probe is the better evidence path for production latency research.
...
loss, drawdown, consecutive-loss, and per-market entry caps. It currently uses
fractional Kelly sizing (`kelly_fraction: 0.35`) and `min_edge: 0.0`, so a trade
is funded only when the model probability beats the executable cost after fees.
